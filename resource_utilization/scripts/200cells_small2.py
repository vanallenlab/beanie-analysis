...
def func(count_path, meta_path):
    bobj = bn.Beanie(counts_path = path+"/data/profiling_datasets/"+count_path, 
                     ...
                     sig_path = path+"/data/signatures/test_small2.gmt",
                     normalised = False, min_cells=50, bins=False, bin_size=20,
                     output_dir = path+"/profiling/results/beanie_out_small2/")
...
for i in range(len(paths_metad)):
    f = open(path+"/profiling/results/test_small2/"+paths_metad[i].split(".")[0]+'.txt', 'w')
    t1_start_wall = time()
